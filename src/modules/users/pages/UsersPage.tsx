...
const UsersPage = () => {
  ...
  const handleFormSubmit = async (data: CreateUserRequest | UpdateUserRequest) => {
    try {
      ...
      if (selectedUser) {
        // Update user
        await userService.updateUser(selectedUser._id || selectedUser.id || '', data);
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
        <Box>
          ...
          <Typography variant="body1" color="text.secondary">
            {isAdmin ? 'Manage users and their permissions' : 'Browse our users list'}
          </Typography>
        </Box>
        {isAdmin && (
          <Button
              ...
              {isAdmin && (
                <CardActions>
                  ...
                  <IconButton
                    ...
                    disabled={actionLoading || (user._id === currentUser?.id || user.id === currentUser?.id)}
                    title={user._id === currentUser?.id || user.id === currentUser?.id ? 'Cannot delete your own account' : 'Delete user'}
                  >
                    <Delete />
                  </IconButton>
                </CardActions>
